...
class Paginator:
    ...
    def _get_kb_paginator_elements(self):
        ...
        if self.current_page == 0:
            keyboard.append(InlineKeyboardButton(text=self._symbol_fill, callback_data='paginator_nothing'))
        else:
            keyboard.append(InlineKeyboardButton(
                text=self._symbol_left, callback_data=f'paginator_open_page_{self.current_page - 1}'))

        keyboard.append(InlineKeyboardButton(text=self._formatted_text_for_button_of_current_page.format(
            page_number=(self.current_page + 1),
            pages_count=self.pages_count
        ...
        if self.current_page + 1 < self.pages_count:
            keyboard.append(InlineKeyboardButton(
                text=self._symbol_right, callback_data=f'paginator_open_page_{self.current_page + 1}'))
        else:
            keyboard.append(InlineKeyboardButton(
                text=self._symbol_fill, callback_data='paginator_nothing'))
        return keyboard
...
router = Router()
_handle_paginator_function = router.callback_query(F.data.startswith('paginator_open_page_'))
_handle_paginator_function(Paginator.open_paginator_other_page_by_callback)
